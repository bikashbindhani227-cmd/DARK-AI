...
async function resolveSubscription(
  ...
  try {
    ...
    if (productObj?.name?.toLowerCase().includes("pentestgpt")) {
      console.info(
        `[Subscription Webhook] Subscription ${subscriptionId} uses legacy PentestGPT product; skipping DARK AI subscription handling`,
      );
...
async function handleSubscriptionDeleted(
  ...
  if (!lookupKey) {
    ...
    if (!resolved) {
      console.info(
        `[Subscription Webhook] subscription.deleted: skipping subscription ${subscription.id} without DARK AI price lookup_key or product fallback for customer ${customerId}`,
      );
      return;
    }
