  ...
  return tool({
    ...
    execute: async (
      ...
      const changedDuringAutoReviewError = (
        ...
        errorResult(
          `Session ${sid} changed while DARK AI was reviewing the action. ${attemptedAction === "send" ? "The input was not sent." : "The session was not killed."} Use action=view to refresh the terminal state, then retry the exact interaction.`,
        );
