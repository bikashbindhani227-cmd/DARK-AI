...
describe("buildProviderOptions fallback chain", () => {
  ...
  it.each(["ask", "agent"] as const)(
    "falls back from DARK AI Pro Grok 4.6 to GLM 5.3 then Kimi K3 in %s mode",
    (mode) => {
      const opts = buildProviderOptions(
        mode === "agent",
        "user-1",
...
describe("isAutoModelSelectionForRetry", () => {
  ...
  it("keeps explicitly selected DARK AI Max Grok 4.6 retryable", () => {
    expect(
      isAutoModelSelectionForRetry({
...
describe("isExplicitDeepSeekProSelectionForRetry", () => {
  it.each(["model-deepseek-v4-pro", "model-deepseek-v4-pro-0813"])(
    "recognizes explicit DARK AI Pro on %s",
    (selectedModel) => {
      expect(
        isExplicitDeepSeekProSelectionForRetry({
          selectedModel,
...
describe("getRetryFallbackModel", () => {
  ...
  it("retries DARK AI Pro Grok with GLM 5.3", () => {
...
describe("resolveServedModelForCostAccounting", () => {
  ...
  it("maps DARK AI Pro primary and fallback usage to their exact cost keys", () => {
    expect(
      resolveServedModelForCostAccounting({
