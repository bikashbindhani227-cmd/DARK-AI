...
describe("first-touch acquisition attribution", () => {
  ...
  it("keeps bounded campaign metadata without URLs or search terms", () => {
    const attribution = createFirstTouchAttribution({
      url: new URL(
        "https://yourdomain.com/?utm_source=newsletter&utm_medium=email&utm_campaign=aug_launch&utm_term=private-target",
      ),
  ...
  it("classifies organic, referral, paid, and direct entry", () => {
    expect(
      createFirstTouchAttribution({
        url: new URL("https://yourdomain.com/download"),
    ...
    expect(
      createFirstTouchAttribution({
        url: new URL("https://yourdomain.com/"),
        referer:
    ...
    expect(
      createFirstTouchAttribution({
        url: new URL("https://yourdomain.com/?ref=SAFE123"),
        referer: null,
        capturedAt,
      }),
    ...
    expect(
      createFirstTouchAttribution({
        url: new URL("https://yourdomain.com/?gclid=opaque-click-id"),
        referer: null,
        capturedAt,
      }),
    ...
    expect(
      createFirstTouchAttribution({
        url: new URL("https://yourdomain.com/"),
        referer: "https://signin.yourdomain.com/callback",
        capturedAt,
      }),
    ).toMatchObject({
      source: "$direct",
  ...
  it("distinguishes assistant referrers, UTM-only markers, and intermediary visits", () => {
    const directReferrer = createFirstTouchAttribution({
      url: new URL("https://yourdomain.com/product"),
      referer: "https://chatgpt.com/c/opaque-conversation-id",
      capturedAt,
    });
    const utmOnly = createFirstTouchAttribution({
      url: new URL("https://yourdomain.com/pricing?utm_source=chatgpt"),
      referer: null,
      capturedAt,
    });
    const intermediary = createFirstTouchAttribution({
      url: new URL("https://yourdomain.com/?utm_source=chatgpt"),
      referer: "https://www.google.com/search?q=hackerai",
      capturedAt,
    });
  ...
  it("round-trips valid values and rejects tampered cookies", () => {
    const attribution = createFirstTouchAttribution({
      url: new URL("https://yourdomain.com/?utm_source=partner"),
  ...
  it("maps attribution to set-once person properties", () => {
    const attribution = createFirstTouchAttribution({
      url: new URL("https://yourdomain.com/trust?utm_source=github"),
