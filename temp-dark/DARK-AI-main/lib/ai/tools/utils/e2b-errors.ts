...
export function getUserFacingE2BErrorMessage(error: unknown): string | null {
  ...
  if (error instanceof AuthenticationError) {
    return "Sandbox authentication failed. The E2B API key may be invalid or expired. Please contact DARK AI support.";
  }
  ...
  if (error instanceof TemplateError) {
    return "Sandbox template is incompatible. Please contact DARK AI support.";
  }
  if (error instanceof TimeoutError) {
  ...
  if (error instanceof InvalidArgumentError) {
    return "Invalid sandbox configuration. Please contact DARK AI support.";
  }
