...
describe("SidebarHeaderContent", () => {
  ...
  it("shows the sidebar button instead of the DARK AI icon when collapsed", () => {
    render(
  ...
  it("shows only the DARK AI mark for free users", () => {
    render(
      <SidebarHeaderContent
        handleCloseSidebar={jest.fn()}
        isCollapsed={false}
    ...
    const homeLink = screen.getByRole("link", { name: "DARK AI home" });

    expect(homeLink).toContainElement(screen.getByTestId("hackerai-svg"));
    expect(screen.queryByText("DARK AI")).not.toBeInTheDocument();
  ...
  it.each<[SubscriptionTier, string]>([
    ...
    expect(
      screen.getByRole("link", { name: `DARK AI ${label} home` }),
    ).toBeInTheDocument();
    expect(screen.queryByTestId("hackerai-svg")).not.toBeInTheDocument();
    expect(screen.getByText("DARK AI")).toBeInTheDocument();
