...
describe("POST /api/subscription/webhook", () => {
  ...
  it("skips legacy PentestGPT invoices before resolving the old product as a DARK AI tier", async () => {
    mockConstructEvent.mockReturnValue({
  ...
  it("deactivates referral paid eligibility for deleted DARK AI subscriptions resolved from product fallback", async () => {
    mockGetReferralRewardConfig.mockReturnValue({
      enabled: true,
      referrerRewardDollars: 10,
    });
  ...
  it("skips deleted legacy PentestGPT subscriptions that do not have a DARK AI price lookup key", async () => {
    mockConstructEvent.mockReturnValue({
