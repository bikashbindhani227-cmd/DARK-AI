...
describe("shouldDropExpectedFrontendException", () => {
                ...
                frames: [
                  {
                    raw_frame: {
                      filename:
                        "https://yourdomain.com/_next/static/chunks/27au5l1vw34oq.js?dpl=dpl_2ruEPyNtAD3Yc4qkCoAN2J7ReZk7",
                    },
                  },
                ],
              },
  ...
  it("adds structured diagnostics to retained frontend exceptions", () => {
    const event = enrichFrontendExceptionEvent({
      event: "$exception",
      properties: {
        $current_url: "https://yourdomain.com/c/chat-123",
        $exception_values: [
              ...
              frames: [
                {
                  raw_frame: {
                    filename:
                      "https://yourdomain.com/_next/static/chunks/react.js?dpl=dpl_G8NpYMgn7xvPfdr4XW6jYBmeC4yB",
                  },
                },
              ],
            },
  ...
  it("removes query strings and fragments from retained exception URLs", () => {
    const event = sanitizeFrontendExceptionUrlProperties({
      ...
      properties: {
        $current_url:
          "https://yourdomain.com/auth-error?state=secret#client_redirect_key=secret",
    ...
    expect(event.properties).toEqual({
      $current_url: "https://yourdomain.com/auth-error",
  ...
  it("does not change URL properties on non-exception events", () => {
    const event = {
      event: "custom_event",
      properties: {
        $current_url: "https://yourdomain.com/c/chat-123?tab=files",
        $referrer: "https://yourdomain.com/?source=home",
      },
    };
